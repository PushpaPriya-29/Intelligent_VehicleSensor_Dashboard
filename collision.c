    ...
    while (1) {
        if (a.brake_status == 0) {
            ...
            if (c.distance < 30 && c.relativeSpeed > 50) {
                c.collisionflag = 1;
                add_alert(alertshm, "\U0001F6A8 COLLISION WARNING!", CRITICAL);
            } else {
...
void* brake(void* arg) {
    while (1) {
        ...
        if (c.relativeSpeed > 80 || c.collisionflag == 1) {
            a.brake_status = 1;
            add_alert(alertshm, "\U0001F6D1 AUTO-BRAKE ENGAGED!", WARNING);
            c.relativeSpeed -= 5;
...
void* lane(void* arg) {
    ...
    while (1) {
        ...
        if (l.laneflag == 1) {
            ...
            add_alert(alertshm, "\U000026A0 LANE DEPARTURE DETECTED!", WARNING);
        } else {
