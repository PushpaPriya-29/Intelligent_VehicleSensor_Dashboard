...
void* environmental_monitor(void* arg) {
    while (1) {
        ...
        if (data.temp < 20 || data.humid < 30) {
            add_alert(alertshm, "\U0001F321 Temperature/Humidity LOW!", WARNING);
            data.env_flag = 1;
        } else if (data.temp > 50 || data.humid > 70) {
            add_alert(alertshm, "\U0001F321 Temperature/Humidity HIGH!", WARNING);
            data.env_flag = 1;
        } else {
        ...
        // Check rain level
        if (data.rain > 70) {
            add_alert(alertshm, "\U0001F327 HEAVY RAIN DETECTED!", WARNING);
            data.env_flag = 1;
        } else {
        ...
        // Check fog level
        if (data.fog > 60) {
            add_alert(alertshm, "\U0001F32B DENSE FOG DETECTED!", WARNING);
            data.env_flag = 1;
        } else {
